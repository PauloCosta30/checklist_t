...
from scrapers.casasbahia import scrape_casasbahia
...
def formatar_alerta(produto: dict, categoria: dict, motivo: str, desconto_pct: float) -> str:
    ...
    # Urgência por nível de desconto
    if desconto_pct >= 70:
    ...
    # Linha de preço original (só mostra se existir)
    linha_original = ""
    ...
    for cat_key, cat_info in CATEGORIAS.items():
        for keyword in cat_info["keywords"]:
            ...
            tasks = [
                ...
                scrape_casasbahia(keyword, cat_info["preco_max"]),
                scrape_magalu(keyword, cat_info["preco_max"]),
            ]
            ...
            for resultado in resultados:
                ...
                for produto in resultado:
                    ...
                    msg = formatar_alerta(produto, cat_info, motivo, desconto_pct)
                    alertas.append(msg)

                    logger.info(
                        f"💥 ERRO: {produto.get('nome','?')[:40]} | "
                        f"{desconto_pct:.0f}% OFF | "
                        f"R${produto.get('preco',0):.2f} | {motivo}"
                    )

            await asyncio.sleep(Config.REQUEST_DELAY + random.uniform(0.5, 1.5))

    # Limpar seen_ids se ficar grande demais
